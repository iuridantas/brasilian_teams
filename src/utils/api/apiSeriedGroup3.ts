import {
  ...
  export const api = {
    getTeamsSerieDGroup3: async (): Promise<Teams[] | undefined> => {
      try {
        const teams = await axios.get('/seried/grupo3', {headers:{'Authorization':'Bearer ' + localStorage.getItem('token')}});
        return teams.data;
      } catch (err: any) {
        handleError(
          'Erro no servidor',
    ...
    deleteTeamSerieDGroup3: async (
      ...
      try {
        const isDeleted = await axios.delete('/seried/grupo3/delete/' + teamId, {headers:{'Authorization':'Bearer ' + localStorage.getItem('token')}});
    ...
    creatTeamSerieDGroup3: async (teams: TeamsInput): Promise<Teams | undefined> => {
      try {
        const newTeam = await axios.post('/seried/grupo3/create', teams, {headers:{'Authorization':'Bearer ' + localStorage.getItem('token')}});
        return newTeam.data;
      } catch (err: any) {
        console.log(err);
    ...
    getTeamSerieDGroup3ById: async (teamId: string): Promise<Teams | undefined> => {
      try {
        const teams = await axios.get('/seried/grupo3/find/' + teamId, {headers:{'Authorization':'Bearer ' + localStorage.getItem('token')}});
        return teams.data;
      } catch (err) {
        handleError(
          'Time não encontrado',
