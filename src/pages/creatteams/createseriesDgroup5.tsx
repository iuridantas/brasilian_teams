...
export function CreateseriesDGroup5() {
  const navigate = useNavigate();
  const [teams, setTeams] = useState<Teams>();
  const { _id } = useParams();

  useEffect(() => {
    getTeamSerieDGroup5ById();
  }, []);

  async function getTeamSerieDGroup5ById() {
    if (_id) {
      const team = await api.getTeamSerieDGroup5ById(_id);
      setTeams(team);
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (_id) {
      ...
      TeamResponse = await api.updateTeamSerieDGroup5(teamToUpdate);
      console.log(TeamResponse);
    } else {
      TeamResponse = await api.creatTeamSerieDGroup5(newTeam);
    }
