...
export function CreateseriesDGroup2() {
  const navigate = useNavigate();
  const [teams, setTeams] = useState<Teams>();
  const { _id } = useParams();

  useEffect(() => {
    getTeamSerieDGroup2ById();
  }, []);

  async function getTeamSerieDGroup2ById() {
    if (_id) {
      const team = await api.getTeamSerieDGroup2ById(_id);
      setTeams(team);
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (_id) {
      ...
      TeamResponse = await api.updateTeamSerieDGroup2(teamToUpdate);
      console.log(TeamResponse);
    } else {
      TeamResponse = await api.creatTeamSerieDGroup2(newTeam);
    }
