...
export function CreateseriesDGroup8() {
  const navigate = useNavigate();
  const [teams, setTeams] = useState<Teams>();
  const { _id } = useParams();

  useEffect(() => {
    getTeamSerieDGroup8ById();
  }, []);

  async function getTeamSerieDGroup8ById() {
    if (_id) {
      const team = await api.getTeamSerieDGroup8ById(_id);
      setTeams(team);
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (_id) {
      ...
      TeamResponse = await api.updateTeamSerieDGroup8(teamToUpdate);
      console.log(TeamResponse);
    } else {
      TeamResponse = await api.creatTeamSerieDGroup8(newTeam);
    }
