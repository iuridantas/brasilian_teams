...
export function CreateseriesDGroup4() {
  ...
  useEffect(() => {
    getTeamSerieDGroup4ById();
  }, []);

  async function getTeamSerieDGroup4ById() {
    if (_id) {
      const team = await api.getTeamSerieDGroup4ById(_id);
      setTeams(team);
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (_id) {
      ...
      TeamResponse = await api.updateTeamSerieDGroup4(teamToUpdate);
      console.log(TeamResponse);
    } else {
      TeamResponse = await api.creatTeamSerieDGroup4(newTeam);
      setLoading(false);
    }
