...
import { api } from '../../utils/api/apiSeriedGroup7';

export function CreateseriesDGroup7() {
  ...
  useEffect(() => {
    getTeamSerieDGroup7ById();
  }, []);

  async function getTeamSerieDGroup7ById() {
    if (_id) {
      const team = await api.getTeamSerieDGroup7ById(_id);
      setTeams(team);
    }
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (_id) {
      ...
      TeamResponse = await api.updateTeamSerieDGroup7(teamToUpdate);
      console.log(TeamResponse);
    } else {
      TeamResponse = await api.creatTeamSerieDGroup7(newTeam);
      setLoading(false);
    }
